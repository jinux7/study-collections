...
class Html extends NuxObject {
  public ctx: CanvasRenderingContext2D;
  public type: string = 'html'; // svg标识符
  public html: string; // svg字符串
  public _element: HTMLImageElement; // svg转化的img element
  public loaded: boolean = false; // 图片是否已经下载
  constructor(options: any) {
    super(options);
    this.html = options.html;
    this._parseHTMLToDataURL(this.html, options).then(base64=> {
      Util.loadImage(base64).then((imgEle: HTMLImageElement) => {
        this._initHtml(imgEle, options);
        this.loaded = true;
        this.ctx&&this.render(this.ctx); // this.ctx有的话再绘制，也就是Nuxcas.add之后
      });
    });
  }
  private _initHtml(imgEle: HTMLImageElement, options: any) {
  ...
  private _parseHTMLToDataURL(html: string, opts: { width: number, height: number }): Promise<string> {
