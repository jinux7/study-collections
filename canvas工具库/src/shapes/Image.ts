...
class Image extends NuxObject {
  ...
  _initElement(element: HTMLImageElement, options: any) {
      this.setElement(element, options);
  }
  setElement(element: HTMLImageElement, options: any) {
  ...
  public fromURL(url: string, callback: (arg: any)=> void, imgOptions: any) {
      Util.loadImage(url).then((img) => {
          callback && callback(new Image(img as HTMLImageElement, imgOptions));
      });
  }
  static async = true;
}
