<template>
  <div class="nux-leftslip">
    <div class="nux-leftslip-item" ref="slipItem" :class="{ 'leftslip-open': isOpen }">
      <div class="nux-leftslip-item-main" @touchstart="touchStart($event)" @touchmove="touchMove($event)" @touchend="touchEnd($event)">
        <slot name="slip-main"></slot>
      </div>

      <div class="nux-leftslip-item-btn" ref="right">
        <slot name="slipbtns">
          <!-- <a class="nux-delet-btn" @click.prevent="onlyDelClick($event)" v-if="onlyDelBtn">删除</a> -->
        </slot>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    touchEnd(e) {
      ...
      if (!this.isClickBack && disX === 0) {
        ...
        if (-disX > 50) {
          ...
          parentElement.className = 'nux-leftslip-item leftslip-open';
          parentElement.dataset.type = 1;
        } else {
          // 向左滑动未超过阙值，或向右滑动时，回原位
          distance = 0;
          parentElement.className = 'nux-leftslip-item';
    ...
    restSlide() {
      let listItems = document.querySelectorAll('.nux-leftslip-item.leftslip-open');

      // 复位
      for (let i = 0; i < listItems.length; i++) {
        listItems[i].style = 'transform:translateX(0px)';
        listItems[i].dataset.type = 0; //是否展开标志位默认0，左滑展开为1，右滑隐藏为0
        listItems[i].className = 'nux-leftslip-item';
        this.isOpen = false;
      }
    }
  }
